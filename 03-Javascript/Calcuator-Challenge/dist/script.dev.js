...
var currentArray = []; //   display of sum and total saved in variables

var display = document.getElementById("total");
var equals = document.getElementById("equals"); //   variable to layer take active operand from user's entry

var finalOperand = ""; // selects all number buttons then operand buttons
...
function showOperand(event) {
  ...
  currentArray.push(event.target.value);
  display.innerHTML = currentArray.join(""); // filters currentarray to take the value of the operand selected by user

  finalOperand = currentArray.filter(function (currentArray) {
...
function calculateTotal(event) {
  ...
  var calculateStr = currentArray.join(""); //   creates variable for the 2 separate parts & operand

  var operandPos = calculateStr.indexOf(finalOperand); //   creates substring of value upto finaloperand

  var firstNum = calculateStr.substring(0, operandPos); //   as above but takes second value

  var secondNum = calculateStr.substring(operandPos + 1); //   carry out the sum depending on final operand value
...
equals.addEventListener("click", calculateTotal); // AC clear button
